# Porcupine Wake Word Engine + Rhino Speech-To-Intent Engine Demo for Arduino (French language)
...
3. Search for `Picovoice_FR`, then click `INSTALL`.
...
1. Open `File`->`Examples`->`Picovoice_FR`->`MicrocontrollerVoiceAssistantExample`.
...
1. Compile and upload the `Porcupine_FR/GetUUID` sketch from the `File -> Examples` menu. Copy the UUID of the board printed at the beginning of the session to the serial monitor.
...
1. Compile and upload the `Rhino_FR/GetUUID` sketch from the `File -> Examples` menu. Copy the UUID of the board printed at the beginning of the session to the serial monitor.
